...
//	}
    private INeedAV iNeedAV;
    private DownloadInfoPanel downPanel;

    private String url;

    private String formattedTitle;
    private String avid_qn;

    public DownloadRunnable(VideoInfo avInfo, ClipInfo clip, int qn) throws Exception {
        this.avInfo = avInfo;
        this.displayName = clip.getAvTitle() + "p" + clip.getRemark() + "-" + clip.getTitle();
        this.clip = clip;
        this.avid = clip.getAvId();
...
//			return;
            throw new Exception("已经下载过 " + record);
        }

        // 新建下载部件
        downPanel = new DownloadInfoPanel(clip, qn);
        // 判断是否在下载任务中
        if (Global.downloadTaskList.get(downPanel) != null) {
            throw new Exception("已经存在相关下载");
        }
        // 查询下载链接
        iNeedAV = new INeedAV();
        String urlQuery;
        ...
        url = urlQuery;
        // 生成格式化名称
        formattedTitle = CmdUtil.genFormatedName(
                avInfo, clip, realQN);
        avid_qn = avid + "-" + realQN;
